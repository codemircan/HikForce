...
# #####################################################################################################################
# HIKVISION IP KAMERA GÜVENLİK DENETİM ARACI
#
# UYARI:
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="HIKVISION IP Kamera Varsayılan Kimlik Bilgisi Denetim Aracı",
        epilog="ÖRNEK KULLANIM:\n"
               "python hikvision_auditor.py iplist.txt\n"
               "python hikvision_auditor.py iplist.txt --scheme https --user guest --password guest123",
    ...
    print_status("HIKVISION Güvenlik Denetim Aracı Başlatıldı...", Fore.CYAN)
    ...
    print_status(f"Tespit Edilen HIKVISION Cihazı: {hikvision_devices}", Fore.GREEN if hikvision_devices > 0 else Fore.WHITE)
...
def scan_target(base_url, user, password):
    ...
    (is_hikvision, is_vulnerable) tuple'ı döndürür.
    """
    is_hik = is_hikvision(base_url)
    if is_hik:
        print_status(f"[!] HIKVISION Cihazı Tespit Edildi: {base_url}", Fore.GREEN)
        is_vuln = check_credentials(base_url, user, password)
        return True, is_vuln
    else:
        print_status(f"[-] {base_url} bir HIKVISION cihazı olarak tanımlanamadı. Atlanıyor.", Fore.YELLOW)
        return False, False
...
def is_hikvision(base_url):
    """
    Bir hedefin HIKVISION cihazı olup olmadığını birden fazla yöntemle doğrular.
    """
